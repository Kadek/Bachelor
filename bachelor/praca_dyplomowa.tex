...
\begin{figure}
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.55]{\ImgPath/rys/loan_processing.pdf}
	}%
	\caption{Loan processing flow}
...
\begin{figure}
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.55]{\ImgPath/rys/loan_taker.pdf}
	}%
...
\begin{figure}
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.55]{\ImgPath/rys/loan_giver.pdf}
	}%
...
\begin{figure}
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.55]{\ImgPath/rys/authority.pdf}
	}%
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.7]{\ImgPath/rys/high_level.png}
\end{center}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.7]{\ImgPath/rys/contracts.png}
\end{center}
...
\textbf{fxml files} - are to JavaFX what HTML files are to web browsers. They use XML to define the structure of a window. CSS files are attached to them to define styles. Finally, each fxml has a controller assigned to it. Every component of the window can assigned some event and the event handler in the controller. The fxml files express great modularity capabilities. They can include other fxml files, which have their own controller. For example, every main window includes a navBar.fxml which handles switches between windows. This way a lot of redundancy can be avoided. In general, the material design rules were tried to be followed as to make the GUI as aesthetically pleasing as possible.
...
Although the software has been successfully deployed and tested there are many aspects which could have been researched more thoroughly. For example, most of the tests were written as a way of speeding up development by creating snapshots of a properly functioning system. If the product were to be release to the public great measures should be put into maximizing the test coverage. Additionally, integration testing has been accomplished by manual means. Proper software development life cycle would require an automatic set of end to end tests. Moreover, as a consequence of putting the first steps in the world of distributed ledgers technology many parts of the implementation had to be taken as a given. What followed is that gas optimization, security or design patterns had to be taken from outside sources as given. Lack of experience made it impossible to assure whether the given solutions are sound. Lastly and most importantly no performance tests have been made making it impossible to estimate the throughoutput of the system.

All of the mentioned problems led to many constrains on the usage of the system in its actual state. Those could be split into two groups. The first consists of constrains natural to blockchain. That is, the amount of transactions per second done on our application is bounded from the top by the amount possible to process on the Ethereum blockchain. The second one comes from the technical debt gathered throughout the development process. That is, future changes to the software could be harder to implement as a result of missing tests and no continuous integration tools. This implies constraints on the quality presented to the end user.

Summarising, a clear plan for future developments can be put together. Foremost, proper set of continuous build, integration and deployment tools should be put together as to make the development process as scalable as it's possible. After that, maximal test coverage should be aimed for so that proper functioning of the system can be guaranteed. Lastly, the system should be deployed on one of the test Ethereum networks so that behaviour of the decentralized application on a blockchain used by others could be researched. Finally, with a product ready for use by customers market actions consisting of selling the product should be undertaken. But this topic reaches outside of this thesis.
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/create_ledger.png}
\end{center}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/create_deposit.png}
\end{center}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/create_bid.png}
\end{center}
	\caption{Bid creator}
	\label{create bid}
\end{figure}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/check_exchange.png}
\end{center}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/check_loans.png}
\end{center}
...
\begin{figure}[!htbp]
	\begin{center}
\centering
\includegraphics[scale=0.75]{\ImgPath/rys/check_loan.png}
\end{center}
...
\begin{figure}[!htbp]
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.8]{\ImgPath/rys/check_consume.png}
	}%
...
\begin{figure}[!htbp]
	\makebox[\textwidth][c]{
		\includegraphics[scale=0.8]{\ImgPath/rys/check_deposit.png}
	}%
...
\bibitem{bitcoin_trans}{Croman Kyle, Eyal Ittay, "On Scaling Decentralized Blockchains", Access date: 24.11.2018, \url{https://link.springer.com/chapter/10.1007%2F978-3-662-53357-4_8}}
\bibitem{eth_trans}{Access date: 24.11.2018, \url{https://www.coindesk.com/information/will-ethereum-scale}}
